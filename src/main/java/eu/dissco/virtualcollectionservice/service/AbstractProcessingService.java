...
public class AbstractProcessingService {
	...
	protected void addVirtualCollection(DigitalSpecimen digitalSpecimen, String virtualCollectionId,
		...
		if (relationships.stream()
			.anyMatch(relationship -> relationship.getDwcRelationshipOfResource().equals(HAS_VIRTUAL_COLLECTION)
			...
			relationships.add(new EntityRelationship().withType("ods:EntityRelationship")
				...
				.withOdsHasAgents(List.of(new Agent().withId(applicationProperties.getPid())
					...
					.withOdsHasIdentifiers(List.of(new Identifier().withId(applicationProperties.getPid())
