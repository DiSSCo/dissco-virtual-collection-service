...
public class ElasticSearchQueryParser {
	...
	private static Query getTermQuery(List<Object> predicateValue, String sanitizedKey) {
		return new Query.Builder()
			.term(t -> t.field(sanitizedKey).value(FieldValue.of(predicateValue.getFirst())).caseInsensitive(true))
			.build();
	}
	...
	private static Query handleSinglePredicate(TargetDigitalObjectFilter filter) {
		...
		return getQueryFromPredicate(predicateType.value(), filter.getOdsPredicateKey(), harmonizedValues);
	}
	...
	private static Query handleMultiplePredicates(TargetDigitalObjectFilter filter) {
		...
		filter.getOdsHasPredicates()
			.stream()
			.map(predicate -> getQueryFromPredicate(predicate.getOdsPredicateType().value(),
	...
	private static Query getQueryFromPredicate(String predicateType, String predicateKey,
		...
		if (predicateType.equals(OdsPredicateType.NOT.value()) && predicateValues.size() == 1) {
		...
		else if (predicateType.equals(OdsPredicateType.EQUALS.value()) && predicateValues.size() == 1) {
			var termQuery = getTermQuery(predicateValues, sanitizedKey);
			...
			return new Query.Builder()
				.terms(t -> t.field(sanitizedKey)
					.terms(ts -> ts
						.value(predicateValues.stream().map(value -> FieldValue.of((String) value)).toList())))
				.build();
		}
		else {
	...
	/**
	 * Assumption: All values in the predicateValue list are of the same type. The method
	 * checks the type of the first element to determine whether to append .keyword to the
	 * sanitized key (for String values). If the list contains mixed types, this may
	 * result in incorrect query construction or runtime errors. Callers must ensure that
	 * predicateValue contains only elements of a single type, and that the first element
