...
class ElasticSearchRepositoryIT {
	...
	void testRetrieveObjects() throws IOException {
		...
		// When
		var specimen = elasticRepository.retrieveObjects(null, DIGITAL_SPECIMEN_INDEX, givenElasticQuery());

		// Then
		assertThat(specimen.getFirst()).isEqualTo(givenSpecimenNode());
	}

	private void postDigitalSpecimens(List<JsonNode> jsonObjects) throws IOException {
		var bulkRequest = new BulkRequest.Builder();
		for (var jsonObject : jsonObjects) {
			bulkRequest.operations(op -> op.index(idx -> idx.index(DIGITAL_SPECIMEN_INDEX)
