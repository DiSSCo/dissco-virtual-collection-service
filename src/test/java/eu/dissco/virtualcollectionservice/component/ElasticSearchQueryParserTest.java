		...
		return Stream.of(
					...
					.of(givenNotFilter(),
							new Query.Builder()
								.bool(b -> b
									.mustNot(
											new Query.Builder()
												.term(t -> t.field("ods:physicalSpecimenIDType.keyword")
													.value(FieldValue.of("Local"))
													.caseInsensitive(true))
												.build()))
								.build()),
				Arguments.of(givenInFilter(),
						new Query.Builder()
							.terms(t -> t.field("ods:topicDiscipline.keyword")
								.terms(ts -> ts.value(List.of(FieldValue.of("Botany"), FieldValue.of("Zoology")))))
							.build()),
					...
					.of(givenAndFilter(),
							new Query.Builder()
								.bool(b -> b.must(List.of(
										new Query.Builder()
											.bool(b2 -> b2.must(m -> m.term(t -> t.field("ods:isKnownToContainMedia")
												.value(FieldValue.of(false))
												.caseInsensitive(true))))
											.build(),
										new Query.Builder()
											.bool(b2 -> b2.must(m -> m.term(t -> t.field("dwc:preparations.keyword")
				...
				Arguments
					.of(givenOrFilter(),
							new Query.Builder().bool(
									b -> b.should(List.of(
											new Query.Builder().bool(b2 -> b2
												.must(m -> m.term(t -> t.field("ods:topicDiscipline.keyword")
													.value(FieldValue.of("Botany"))
													.caseInsensitive(true))))
												.build(),
											new Query.Builder()
												.bool(b2 -> b2.must(m -> m.term(t -> t.field("ods:version")
				...
				Arguments.of(new TargetDigitalObjectFilter().withOdsPredicateType(OdsPredicateType.AND)
					.withOdsHasPredicates(List.of(new OdsHasPredicate()
						...
						new Query.Builder().bool(b -> b.must(List.of(
								new Query.Builder()
									.bool(b2 -> b2.must(m -> m.term(t -> t
										.field("ods:hasIdentifications.ods:hasTaxonIdentifications.dwc:kingdom.keyword")
										.value(FieldValue.of("Plantae"))
										.caseInsensitive(true))))
									.build(),
								new Query.Builder().bool(b2 -> b2.must(m -> m.term(t -> t
									.field("ods:hasEvents.ods:hasLocation.ods:hasGeoreference.dwc:decimalLatitude")
